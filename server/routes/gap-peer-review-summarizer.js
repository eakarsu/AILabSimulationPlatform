...
// Route: /api/gap-peer-review-summarizer
// Feature: Peer Review Summarizer
// Description: (synthesize peer feedback)
  ...
  try {
    const r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: 'system', content: 'You are an expert assistant for: Peer Review Summarizer. (synthesize peer feedback)' },
...
router.post('/', async (req, res) => {
  try {
    ...
    const prompt = `Feature: Peer Review Summarizer\nDescription: (synthesize peer feedback)\nInput: ${input}\nContext: ${JSON.stringify(context || {})}`;
    const result = await callOpenRouter(prompt);
    // Best-effort persistence
    try {
      if (db && typeof db.query === 'function') {
        await db.query('INSERT INTO gap_features (slug, input, output, meta) VALUES ($1, $2, $3, $4)',
          ['peer-review-summarizer', input, JSON.stringify(result), JSON.stringify(context || {})]);
    ...
    return res.json({ feature: 'peer-review-summarizer', title: 'Peer Review Summarizer', result, generatedAt: new Date().toISOString() });
  } catch (e) {
...
router.get('/history', async (req, res) => {
  try {
    const db = req.app && (req.app.get ? req.app.get('db') : null);
    if (!db) return res.json({ slug: 'peer-review-summarizer', items: [] });
    if (typeof db.query === 'function') {
      const r = await db.query('SELECT id, input, output, created_at FROM gap_features WHERE slug = $1 ORDER BY id DESC LIMIT 20', ['peer-review-summarizer']);
      return res.json({ slug: 'peer-review-summarizer', items: r.rows || [] });
    }
    return res.json({ slug: 'peer-review-summarizer', items: [] });
  } catch (e) {
    return res.json({ slug: 'peer-review-summarizer', items: [], error: (e && e.message) || 'Server error' });
  }
});
