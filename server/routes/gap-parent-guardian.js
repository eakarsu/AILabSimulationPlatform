...
// Route: /api/gap-parent-guardian
// Feature: Parent Guardian
// Description: Parent/guardian progress notifications
  ...
  try {
    const r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: 'system', content: 'You are an expert assistant for: Parent Guardian. Parent/guardian progress notifications' },
...
router.post('/', async (req, res) => {
  try {
    ...
    const prompt = `Feature: Parent Guardian\nDescription: Parent/guardian progress notifications\nInput: ${input}\nContext: ${JSON.stringify(context || {})}`;
    const result = await callOpenRouter(prompt);
    // Best-effort persistence
    try {
      if (db && typeof db.query === 'function') {
        await db.query('INSERT INTO gap_features (slug, input, output, meta) VALUES ($1, $2, $3, $4)',
          ['parent-guardian', input, JSON.stringify(result), JSON.stringify(context || {})]);
    ...
    return res.json({ feature: 'parent-guardian', title: 'Parent Guardian', result, generatedAt: new Date().toISOString() });
  } catch (e) {
...
router.get('/history', async (req, res) => {
  try {
    const db = req.app && (req.app.get ? req.app.get('db') : null);
    if (!db) return res.json({ slug: 'parent-guardian', items: [] });
    if (typeof db.query === 'function') {
      const r = await db.query('SELECT id, input, output, created_at FROM gap_features WHERE slug = $1 ORDER BY id DESC LIMIT 20', ['parent-guardian']);
      return res.json({ slug: 'parent-guardian', items: r.rows || [] });
    }
    return res.json({ slug: 'parent-guardian', items: [] });
  } catch (e) {
    return res.json({ slug: 'parent-guardian', items: [], error: (e && e.message) || 'Server error' });
  }
});
