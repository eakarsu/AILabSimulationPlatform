...
// Route: /api/gap-student-misconception-detector
// Feature: Student Misconception Detector
// Description: (cluster common errors)
  ...
  try {
    const r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: 'system', content: 'You are an expert assistant for: Student Misconception Detector. (cluster common errors)' },
...
router.post('/', async (req, res) => {
  try {
    ...
    const prompt = `Feature: Student Misconception Detector\nDescription: (cluster common errors)\nInput: ${input}\nContext: ${JSON.stringify(context || {})}`;
    const result = await callOpenRouter(prompt);
    // Best-effort persistence
    try {
      if (db && typeof db.query === 'function') {
        await db.query('INSERT INTO gap_features (slug, input, output, meta) VALUES ($1, $2, $3, $4)',
          ['student-misconception-detector', input, JSON.stringify(result), JSON.stringify(context || {})]);
    ...
    return res.json({ feature: 'student-misconception-detector', title: 'Student Misconception Detector', result, generatedAt: new Date().toISOString() });
  } catch (e) {
...
router.get('/history', async (req, res) => {
  try {
    const db = req.app && (req.app.get ? req.app.get('db') : null);
    if (!db) return res.json({ slug: 'student-misconception-detector', items: [] });
    if (typeof db.query === 'function') {
      const r = await db.query('SELECT id, input, output, created_at FROM gap_features WHERE slug = $1 ORDER BY id DESC LIMIT 20', ['student-misconception-detector']);
      return res.json({ slug: 'student-misconception-detector', items: r.rows || [] });
    }
    return res.json({ slug: 'student-misconception-detector', items: [] });
  } catch (e) {
    return res.json({ slug: 'student-misconception-detector', items: [], error: (e && e.message) || 'Server error' });
  }
});
