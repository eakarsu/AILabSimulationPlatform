// Auto-generated feature route: peer-feedback-synthesis
// Domain: peer feedback synthesizer — Capability: synthesizing peer comments into constructive student feedback
// Mounted under: /api/peer-feedback-synthesis
...
// POST /api/peer-feedback-synthesis/run — execute the feature
router.post('/run', async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = 'You are a peer feedback synthesizer. Your task is synthesizing peer comments into constructive student feedback. Respond with concise JSON: { "summary": string, "recommendations": string[], "next_actions": string[], "confidence": number }.';
...
// GET /api/peer-feedback-synthesis/history — recent runs
router.get('/history', (req, res) => {
  const limit = Math.min(parseInt(req.query.limit, 10) || 25, 100);
  res.json({ items: store.slice(-limit).reverse() });
});

// GET /api/peer-feedback-synthesis/:id — fetch one
...
// DELETE /api/peer-feedback-synthesis/:id — discard a run
