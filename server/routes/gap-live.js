...
// Route: /api/gap-live
// Feature: Live
// Description: Live chat / shared notebooks during labs (collaboration entity exists but no realtime layer)
  ...
  try {
    const r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: 'system', content: 'You are an expert assistant for: Live. Live chat / shared notebooks during labs (collaboration entity exists but no realtime layer)' },
...
router.post('/', async (req, res) => {
  try {
    ...
    return res.json({ feature: 'live', title: 'Live', result, generatedAt: new Date().toISOString() });
  } catch (e) {
...
router.get('/history', async (req, res) => {
  try {
    const db = req.app && (req.app.get ? req.app.get('db') : null);
    if (!db) return res.json({ slug: 'live', items: [] });
    if (typeof db.query === 'function') {
      const r = await db.query('SELECT id, input, output, created_at FROM gap_features WHERE slug = $1 ORDER BY id DESC LIMIT 20', ['live']);
      return res.json({ slug: 'live', items: r.rows || [] });
    }
    return res.json({ slug: 'live', items: [] });
  } catch (e) {
    return res.json({ slug: 'live', items: [], error: (e && e.message) || 'Server error' });
  }
});
