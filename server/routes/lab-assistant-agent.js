// Auto-generated feature route: lab-assistant-agent
// Domain: adaptive STEM lab tutor — Capability: guiding students through procedural steps with adaptive difficulty
// Mounted under: /api/lab-assistant-agent
...
// POST /api/lab-assistant-agent/run — execute the feature
router.post('/run', async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = 'You are a adaptive STEM lab tutor. Your task is guiding students through procedural steps with adaptive difficulty. Respond with concise JSON: { "summary": string, "recommendations": string[], "next_actions": string[], "confidence": number }.';
...
// GET /api/lab-assistant-agent/history — recent runs
router.get('/history', (req, res) => {
  const limit = Math.min(parseInt(req.query.limit, 10) || 25, 100);
  res.json({ items: store.slice(-limit).reverse() });
});

// GET /api/lab-assistant-agent/:id — fetch one
...
// DELETE /api/lab-assistant-agent/:id — discard a run
