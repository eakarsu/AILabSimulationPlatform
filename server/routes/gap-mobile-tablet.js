...
// Route: /api/gap-mobile-tablet
// Feature: Mobile Tablet
// Description: Mobile/tablet companion
  ...
  try {
    const r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: 'system', content: 'You are an expert assistant for: Mobile Tablet. Mobile/tablet companion' },
...
router.post('/', async (req, res) => {
  try {
    ...
    return res.json({ feature: 'mobile-tablet', title: 'Mobile Tablet', result, generatedAt: new Date().toISOString() });
  } catch (e) {
...
router.get('/history', async (req, res) => {
  try {
    const db = req.app && (req.app.get ? req.app.get('db') : null);
    if (!db) return res.json({ slug: 'mobile-tablet', items: [] });
    if (typeof db.query === 'function') {
      const r = await db.query('SELECT id, input, output, created_at FROM gap_features WHERE slug = $1 ORDER BY id DESC LIMIT 20', ['mobile-tablet']);
      return res.json({ slug: 'mobile-tablet', items: r.rows || [] });
    }
    return res.json({ slug: 'mobile-tablet', items: [] });
  } catch (e) {
    return res.json({ slug: 'mobile-tablet', items: [], error: (e && e.message) || 'Server error' });
  }
});
