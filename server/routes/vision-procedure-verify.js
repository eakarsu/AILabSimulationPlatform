// Auto-generated feature route: vision-procedure-verify
// Domain: lab safety vision inspector — Capability: validating photographed lab setup against expected configuration
// Mounted under: /api/vision-procedure-verify
  ...
  return new Promise((resolve) => {
    ...
    const options = {
      ...
      headers: {
        ...
        'X-Title': 'Lab Simulation Platform'
      }
    };
...
// POST /api/vision-procedure-verify/run — execute the feature
router.post('/run', async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = 'You are a lab safety vision inspector. Your task is validating photographed lab setup against expected configuration. Respond with concise JSON: { "summary": string, "recommendations": string[], "next_actions": string[], "confidence": number }.';
...
// GET /api/vision-procedure-verify/history — recent runs
router.get('/history', (req, res) => {
  const limit = Math.min(parseInt(req.query.limit, 10) || 25, 100);
  res.json({ items: store.slice(-limit).reverse() });
});

// GET /api/vision-procedure-verify/:id — fetch one
...
// DELETE /api/vision-procedure-verify/:id — discard a run
