...
// Route: /api/gap-research-paper-recommender
// Feature: Research Paper Recommender
// Description: (suggest reading per topic)
  ...
  try {
    const r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: 'system', content: 'You are an expert assistant for: Research Paper Recommender. (suggest reading per topic)' },
...
router.post('/', async (req, res) => {
  try {
    ...
    const prompt = `Feature: Research Paper Recommender\nDescription: (suggest reading per topic)\nInput: ${input}\nContext: ${JSON.stringify(context || {})}`;
    const result = await callOpenRouter(prompt);
    // Best-effort persistence
    try {
      if (db && typeof db.query === 'function') {
        await db.query('INSERT INTO gap_features (slug, input, output, meta) VALUES ($1, $2, $3, $4)',
          ['research-paper-recommender', input, JSON.stringify(result), JSON.stringify(context || {})]);
    ...
    return res.json({ feature: 'research-paper-recommender', title: 'Research Paper Recommender', result, generatedAt: new Date().toISOString() });
  } catch (e) {
...
router.get('/history', async (req, res) => {
  try {
    const db = req.app && (req.app.get ? req.app.get('db') : null);
    if (!db) return res.json({ slug: 'research-paper-recommender', items: [] });
    if (typeof db.query === 'function') {
      const r = await db.query('SELECT id, input, output, created_at FROM gap_features WHERE slug = $1 ORDER BY id DESC LIMIT 20', ['research-paper-recommender']);
      return res.json({ slug: 'research-paper-recommender', items: r.rows || [] });
    }
    return res.json({ slug: 'research-paper-recommender', items: [] });
  } catch (e) {
    return res.json({ slug: 'research-paper-recommender', items: [], error: (e && e.message) || 'Server error' });
  }
});
